...
import { mockTickets as initialMockTickets } from "../mockData";
...
export const Inbox = ({ defaultFilter = "All" }: { defaultFilter?: string }) => {
  ...
  const [tickets, setTickets] = React.useState<any[]>(initialMockTickets);
  const [selectedId, setSelectedId] = React.useState<string | null>(ticketIdParam || tickets[0]?.id || null);
  const [activeFilter, setActiveFilter] = React.useState(defaultFilter);
  const [isDrafting, setIsDrafting] = React.useState(false);
  const [isEditing, setIsEditing] = React.useState(false);
  const [aiDrafts, setAiDrafts] = React.useState<Record<string, AIDraftResponse>>({});
  const { toast } = useToast();

  const { documents, businessIdentity, brandVoice } = useAppStore();
