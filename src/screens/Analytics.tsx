      ...
      {channelReplyRates.length > 0 ? (
        <div className="grid grid-cols-4 gap-6 mb-8">
          {channelReplyRates.map(ch => {
            // Was a non-null assertion (`.find(...)!`) — crashed the
            // entire Analytics page if the backend ever sent a channel
            // with no matching entry here, which is exactly what happened
            // live: the backend already maps 'instagram' -> 'Instagram'
            // (computeChannelReplyRates/REPLY_RATE_CHANNEL_DISPLAY), but
            // this list never had an Instagram entry until now. Falling
            // back to a generic style instead of a fixed list of 4
            // assertion means any future channel added on the backend
            // degrades gracefully here instead of blanking the page.
            const cfg = CHANNEL_CFG.find(c => c.id === ch.channel) || {
              id: ch.channel, color: '#8B8B8B', bgClass: 'bg-white/5',
              borderClass: 'border-white/10', textClass: 'text-text-muted',
              icon: <MessageCircle size={14} />,
            };
